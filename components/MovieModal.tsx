...
const MovieModal = () => {
  ...
  const fetchMovieById = async (movieId: number | string | undefined): Promise<void> => {
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie/${movieId}?api_key=ee05cb5c4e7bec8bf2cb81503e07020d`
      );
  ...
  return (
    <div className='fixed z-30 w-screen h-screen bg-white overflow-y-scroll'>
      <div className='flex justify-end'>
        <div className='m-2 p-5 hover:cursor-pointer rounded-full z-40' onClick={handleModalClose}>
          <HiXMark size={26} />
        </div>
      </div>
      <div className='flex flex-col md:flex-row justify-center items-center min-w-[240px] px-10'>
        ...
        <div className='md:w-[50%] flex flex-col justify-center items-center'>
          <div className='flex flex-wrap text-center uppercase mt-6 text-xl lg:text-3xl font-medium'>
            {movieData?.title}
          </div>
          <div className='flex text-xs lg:text-base font-medium mt-4 w-full'>
            ...
            <div className='flex flex-col text-center w-[50%] gap-y-2'>
              <div>
                ...
                /{' '}
                <span className='bg-gray-100 rounded-md py-[2px] px-2'>
